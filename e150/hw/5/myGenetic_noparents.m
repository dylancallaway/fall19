function [PI, Orig, Lambda, best_designs] = myGenetic_noparents(f, interval, parents, TOL_GA, G, S, dv)
% Input parameters
...
for g = 1:G % For each generation
    ...
    pop_temp3 = [];
    for d = 1:dv
        pop_temp2 = interval(d,1) + (interval(d,2)-interval(d,1)) .* rand(1, S-parents);
        pop_temp3(d,:) = pop_temp2;
    end
    pop = [children, pop_temp3];
